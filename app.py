...
# Funzione di ricerca Google Books (senza errori)
...
with col1:
    titolo = st.text_input("Titolo", value=st.session_state.get('temp_libro', {}).get('titolo', ''))
    autore = st.text_input("Autore", value=st.session_state.get('temp_libro', {}).get('autore', ''))
with col2:
    isbn = st.text_input("ISBN", value=st.session_state.get('temp_libro', {}).get('isbn', ''))
    scaffale = st.text_input("Scaffale")

trama = st.text_area("Trama", value=st.session_state.get('temp_libro', {}).get('trama', ''))
...
for l in reversed(libri):
    st.write(f"**{l['titolo']}** - {l['autore']} (Scaffale: {l['scaffale']})")
