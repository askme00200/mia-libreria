...
def cerca_dati_online(isbn_code):
    try:
        ...
        with urllib.request.urlopen(req, timeout=8) as response:
            ...
            autore = data.get("authors", [{}])[0].get("name", "Autore Sconosciuto")
            parti = autore.strip().split()
            cog, nom = (parti[-1], " ".join(parti[:-1])) if len(parti) > 1 else (parti[0], "")
...
with tab2:
    ...
    if st.button("🌟 SALVA IL LIBRO ORA") and ins_titolo and ins_cognome:
        cursor.execute('''INSERT INTO libri (filename, titolo, cognome_autore, nome_autore, isbn, pagine, data_pub, copertina, recensione, scaffale)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', ("Manuale", ins_titolo, ins_cognome.strip(), ins_nome.strip(), "", "N.D.", "N.D.", COPERTINA_DEFAULT, "Nessuna trama.", "Non assegnato"))
        conn.commit(); salva_backup(); st.balloons(); st.rerun()

# --- RICERCA COMPLETA REINSERITA ---
...
with col_c2: cerca_cognome = st.text_input("👤 Per Cognome Autore", key="c_cog")
...
if libri_tutti:
    for row in libri_tutti:
        ...
        with col2:
            ...
            nuovo_scaf = st.text_input(f"Sposta scaffale per '{t}'", value=scaf, key=f"ed_{db_id}")
            if nuovo_scaf != scaf:
